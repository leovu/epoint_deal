...
class PickOneStaffScreen extends StatefulWidget {
  ...
  @override
  _PickOneStaffScreenState createState() => _PickOneStaffScreenState();
}

class _PickOneStaffScreenState extends State<PickOneStaffScreen> {
  ...
  void initState() {
    ...
    super.initState();
    _bloc = PickOneStaffBloc(context, widget.models, widget.staffs);
  ...
  Future _onRefresh() {
    ...
    if (widget.staffs == null) {
      group.add(_bloc.workListStaff(
          widget.models, _controllerSearch.text, widget.projectId));
    }
