...
import '../bloc/ordered_service_bloc.dart';

class OrderedServiceScreen extends StatefulWidget {
  final ServiceNewModel model;
  final CustomerModel? customerModel;
  final BranchModel? branchModel;
  final bool isBooking;

  const OrderedServiceScreen(
      {super.key,
      required this.model,
      this.customerModel,
      this.branchModel,
  ...
  @override
  OrderedServiceScreenState createState() => OrderedServiceScreenState();
}

class OrderedServiceScreenState extends State<OrderedServiceScreen> {
  late OrderedServiceBloc _bloc;

  @override
  void initState() {
    // TODO: implement initState
    super.initState();
    _bloc = OrderedServiceBloc(context);
  ...
  Future _onRefresh({bool isRefresh = true}) {
    final group = <Future>[];
    if(_bloc.isBooking)
      group.add(_bloc.getRoom(isRefresh));
  ...
  Widget build(BuildContext context) {
    return CustomScaffold(
      title: AppLocalizations.text(LangKey.ordered_service),
      body: _buildBody(),
    );
  }
}
