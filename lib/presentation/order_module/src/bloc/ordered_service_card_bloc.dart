...
class OrderedServiceCardBloc extends BaseBloc {
  OrderedServiceCardBloc(BuildContext context) {
    setContext(context);
  }
  ...
  late OrderServiceCardModel model;
  ...
  chooseVoucher() async {
    DiscountCartModel? event = await CustomNavigator.push(
        ...
        OrderDiscountScreen(
          customerModel: customerModel,
          model: ArrObject(
              objectId: model.serviceCardId,
              objectType: subjectTypeServiceCard),
          amount: parseMoney(controllerPrice.text, defaultValue: model.price),
        ));
    if (event != null) {
      voucherModel = event;
      controllerDiscount.text = formatMoney(getDiscount(voucherModel,
          total: parseMoney(controllerPrice.text, defaultValue: model.price) *
